...
╚══════════════════════════════════════════════════════╝
A terminal UI chatbot with an ASCII robot and speech bubbles.
"""
...
ROBOT_WIDTH = max(len(l) for l in ROBOT_LINES)
...
def print_bot_frame(response: str):
    """Print robot ASCII art beside the speech bubble."""
    ...
    for r, b in zip(robot, bub):
        line = r.ljust(ROBOT_WIDTH) + gap + b
        if RICH:
            # Colour the robot cyan, bubble white
            rich_line = Text()
            rich_line.append(r.ljust(ROBOT_WIDTH), style="bold cyan")
            rich_line.append(gap)
            rich_line.append(b, style="bright_white")
...
def animated_thinking():
    ...
    if RICH:
        ...
        for _ in range(3):
            for f in [".", "..", "..."]:
                sys.stdout.write(f"\r  Thinking{f}   ")
                sys.stdout.flush()
                time.sleep(0.3)
